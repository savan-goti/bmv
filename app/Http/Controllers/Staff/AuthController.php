...
class AuthController extends Controller
{
    ...
    public function redirectToGoogle()
    {
        return Socialite::driver('google')
            ->redirectUrl(route('staff.auth.google.callback'))
            ->redirect();
    }
    ...
    public function handleGoogleCallback(Request $request)
    {
        try {
            ...
            $staff = \App\Models\Staff::where('google_id', $googleUser->getId())->first();
            
            if (!$staff) {
                // Check if user exists with this email
                $staff = \App\Models\Staff::where('email', $googleUser->getEmail())->first();
                ...
                if ($staff) {
                    ...
                    // Don't create new staff accounts via Google login
                    return redirect()->route('staff.login')->with('error', 'No staff account found with this email. Please contact the administrator.');
                }
            } else {
            ...
            if ($staff->status != 1) {
                return redirect()->route('staff.login')->with('error', 'Your account is inactive. Please contact the administrator.');
            }
            ...
            return redirect()->route('staff.dashboard')->with('success', 'Successfully logged in with Google!');

        } catch (\Exception $e) {
            \Log::error('Google OAuth Error: ' . $e->getMessage());
            return redirect()->route('staff.login')->with('error', 'Failed to login with Google. Please try again.');
        }
    }
